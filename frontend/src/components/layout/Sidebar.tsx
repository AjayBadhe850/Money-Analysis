...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="w-64 shrink-0 border-r border-slate-800 bg-slate-950/80 backdrop-blur-xl flex flex-col h-screen sticky top-0">
      {/* Brand Header */}
      <div className="h-16 flex items-center gap-3 px-6 border-b border-slate-800/80">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-indigo-600 via-violet-600 to-cyan-400 flex items-center justify-center shadow-lg shadow-indigo-500/25">
          <Sparkles className="w-5 h-5 text-white" />
        </div>
        <div>
          <div className="font-bold text-sm tracking-tight text-white flex items-center gap-1.5">
            Money Analysis <span className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-400 border border-indigo-500/30">AI</span>
          </div>
          <p className="text-[10px] text-slate-400 font-medium">Finance Controller</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3 space-y-4">
        ...
        <div className="space-y-1 pt-2 border-t border-slate-800/80">
          <div className="px-3 pb-1 text-[10px] font-bold uppercase tracking-wider text-indigo-400 flex items-center gap-1">
            <Sparkles className="w-3 h-3 text-indigo-400" /> Multi-Agent AI Suite
          </div>
          {filterRole(aiItems).map((item) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <Icon className="w-4 h-4 transition-transform group-hover:scale-110 text-indigo-400" />
                  <span>{item.label}</span>
                </div>
                {item.badge && (
                  <span className="text-[9px] uppercase px-1.5 py-0.5 rounded bg-indigo-500/15 text-indigo-300 border border-indigo-500/30 font-semibold font-mono">
                    {item.badge}
                  </span>
                )}
              </NavLink>
      ...
      <div className="p-3 border-t border-slate-800/80 bg-slate-900/40">
        <div className="flex items-center gap-2.5 p-2 rounded-xl bg-slate-900/80 border border-slate-800">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-indigo-400 font-semibold truncate">{user?.role || 'Viewer'}</p>
          </div>
        </div>
      </div>
    </aside>
